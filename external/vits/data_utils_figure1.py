...
class TextAudioLoader(torch.utils.data.Dataset):
    ...
    def get_audio(self, filename):
        ...
        spec_filename = filename.replace(
            ".wav",
            ".spec.pt"
        )
...
class TextAudioCollate():
    ...
    def __call__(self, batch):
        ...
        max_wav_len = max(
            [x[3].size(1) for x in batch]
        )
        max_f0_len = max(
            [x[4].size(1) for x in batch]
        )
        ...
        for i in range(
            len(ids_sorted_decreasing)
        ):

            row = batch[
                ids_sorted_decreasing[i]
            ]
